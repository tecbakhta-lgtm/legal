...
### A.2 Data types — declare these two only
...
### B.1 Data types — declare these two only
...
Contact Info · Health & Fitness · Financial Info · Location · Sensitive Info · Contacts · User Content (all sub-types) · Browsing History · Search History · Identifiers → User ID · Purchases · Usage Data (Product Interaction, Advertising Data, Other Usage Data) · Diagnostics → **Performance Data** and Other Diagnostic Data · Surroundings · Body · Other Data.
...
The app-level manifest must declare no tracking, no tracking domains, the two collected data types, and the required-reason API categories used by first-party code. Third-party SDKs (React Native, the Expo modules, Sentry) ship their own privacy manifests, which are aggregated by Xcode at build time and must not be duplicated here.
...
**Justification of the two API entries:** `CA92.1` — user defaults are read and written only to store this app's own settings, accessible only to this app. `C617.1` — file timestamp information is read only for files inside the app's own container, when writing and verifying an exported backup file.

**Configuration note:** the project is managed by Expo and `app.json` currently contains no `ios.privacyManifests` key, so no manifest is generated. Add the declarations above under `expo.ios.privacyManifests` in `app.json` (or place the file in the iOS project after prebuild) before an App Store submission.
...
| No location, contacts, photos, microphone, calendar | No corresponding permission in `app.json` and no corresponding module in `package.json` |
| Permissions | `app.json` → `android.permissions`: `RECEIVE_BOOT_COMPLETED`, `POST_NOTIFICATIONS` only |
